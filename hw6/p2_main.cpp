...
uint16_t sample_top_buffer[SAMPLE_BUFFER_SIZE];
uint32_t sample_btm_buffer[SAMPLE_BUFFER_SIZE];
...
void printTopBuffer(){
    printf("top buffer done\n");
    for(int i =0; i < 128; i++){
        printf("index %d value %d\n", i+1 ,sample_top_buffer[i]);
    }
}

void printBtmBuffer(){
    printf("down buffer done\n");
...
{
    event_queue.call(printBtmBuffer);
}

void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef *hadc)
{
    event_queue.call(printTopBuffer);
}
...
int main()
{
    ...
    TIM1_Init();
    ADC1_DMA1CH1_init();
    HAL_ADC_Start_DMA(&hadc1, (uint32_t *)sample_top_buffer,(uint32_t) 256);
